#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <signal.h>
#include <sys/types.h>
#include <sys/wait.h>

void ssu_signal_handler1(int signo);
void ssu_signal_handler2(int signo);

int main(void) {

	struct sigaction act_int, act_quit;

	act_int.sa_handler = ssu_signal_handler1; //SIGINT에 대한 핸들러 
	sigemptyset(&act_int.sa_mask); //시그널 집합을 비운다
	sigaddset(&act_int.sa_mask, SIGINT); //SIGINT을 집합에 넣음
	act_int.sa_flags = 0;

	if (sigaction(SIGINT, &act_int, NULL) < 0) {
		fprintf(stderr, "sigaction(SIGINT) error\n");
		exit(1);
	}

	act_quit.sa_handler = ssu_signal_handler2; //SIGQUIT에 대한 핸들러
	sigemptyset(&act_quit.sa_mask); //시그널 집합을 비운다
	sigaddset(&act_quit.sa_mask, SIGQUIT); //SIGQUIT을 집합에 넣음
	act_quit.sa_flags = 0;
	
	if (sigaction(SIGQUIT, &act_quit, NULL) < 0) {
		fprintf(stderr, "sigaction(SIGQUIT) error\n");
		exit(1);
	}

	pause(); //일시정지
	exit(0);
}


void ssu_signal_handler1(int signo) { //SIGINT에 대한 핸들러함수

	printf("Signal handler of SIGINT : %d\n" , signo);
	printf("SIGQUIT signal is blocked : %d\n" , signo);
	printf("sleeping 3 sec\n");
	sleep(3);
	printf("Signal handler of SIGINT ended\n");
}

void ssu_signal_handler2(int signo) { //SIGQUIT에 대한 핸들러함수

	printf("Signal handler of SIGQUIT : %d\n" , signo);
	printf("SIGQUIT signal is blocked : %d\n" , signo);
	printf("sleeping 3 sec\n");
	sleep(3);
	printf("Signal handler of SIGQUIT ended\n");
}

